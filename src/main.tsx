...
function App() {
  ...
  function handleDeclareWar(targetId: string, goal: WarGoal) {
    ...
    if (result.error) setAdvisorText(`A guerra não foi iniciada: ${result.message}`);
      ...
      setMapMode('Militar');
      setAdvisorText(`${entity.name} declarou guerra a ${target?.name ?? targetId}. ${result.message} As formações reais agora sofrerão baixas, suprimento e recuos durante o conflito.`);
    }
  }
