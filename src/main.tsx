...
import { StrategicWarConsole } from './components/StrategicWarConsole';
...
function App() {
  ...
  const campaignEntities = useMemo(() => [...scenario.entities, ...emergentScenarioEntities(simulation, scenario.entities)], [scenario, simulation]);
  const seededEntity = useMemo(() => campaignEntities.find((item) => item.id === selectedId), [campaignEntities, selectedId]);
  const entity = mapSelection ?? seededEntity ?? campaignEntities[0] ?? scenario.entities[0];
  const runtime = simulation.entities[entity.id];
  const shownDate = formatDate(simulation.date);
  const entityNames = useMemo(() => Object.fromEntries(campaignEntities.map((item) => [item.id, item.name])), [campaignEntities]);
  ...
  function handleMapCountry(name: string) {
    ...
    const known = campaignEntities.find((item) => item.name.toLowerCase() === name.toLowerCase());
  ...
  function handleTerritory(entityId: string, locationName: string) {
    const known = campaignEntities.find((item) => item.id === entityId);
  ...
  function handleDiplomaticAction(targetId: string, message: string) { const target = campaignEntities.find((item) => item.id === targetId); setSimulation((state) => applyDiplomaticAction(state, entity.id, targetId, message)); setAdvisorText(`Mensagem diplomática enviada de ${entity.name} para ${target?.name ?? targetId}. A negociação altera relação, confiança, memória e tratados da campanha.`); }
  ...
  function handleDeclareWar(targetId: string, goal: WarGoal) {
    const target = campaignEntities.find((item) => item.id === targetId);
